...
export function NewSetupState(guildId: string, config: ITicketConfig): ISetupState {
    return {
        guildId,
        view: "home",
        config,
...
function Home(builder: ComponentV2Builder, state: ISetupState): void {
    ...
    builder.text(
        `${config.enabled ? "🟢 **Aktiv**" : "🔴 **Inaktiv**"}\n` +
            ...
            `📜 **Transcripts:** ${config.transcriptChannelId ? `<#${config.transcriptChannelId}>` : "_noch keiner_"}\n` +
...
function Channels(builder: ComponentV2Builder, state: ISetupState): void {
        ...
        `**Modus:** ${forum ? "Forum-Beiträge" : "Kanäle in einer Kategorie"}\n\n` +
            (forum
                ? "Jedes Ticket wird ein Beitrag im Forum. Die Priorität setzt einen Forum-Tag, " +
                  "geschlossene Tickets werden archiviert statt gelöscht — der Verlauf bleibt durchsuchbar."
                : "Jedes Ticket wird ein eigener Textkanal. Beim Schliessen wird er gelöscht, " +
                  "das Transcript bleibt erhalten.")
    );
    ...
    builder.text(
        `📦 **${forum ? "Forum" : "Kategorie"}:** ${
            ...
            `📜 **Transcript-Kanal:** ${config.transcriptChannelId ? `<#${config.transcriptChannelId}>` : "_noch keiner_"}\n` +
    ...
    builder.buttons(
        ...
        { customId: `${SETUP_PREFIX}:pick:transcript`, label: "Transcript-Kanal", emoji: "📜", tone: "primary" },
...
function Picker(builder: ComponentV2Builder, state: ISetupState): void {
    const labels: Record<string, string> = {
        ...
        transcript: "den Transcript-Kanal",
...
function Roles(builder: ComponentV2Builder, state: ISetupState): void {
    const { config } = state;
    ...
    if (config.supportRoleIds.length > 0) {
        builder.select({
            ...
            options: config.supportRoleIds.map((roleId) => ({ label: `Rolle ${roleId}`, value: roleId, emoji: "🛠️" })),
        });
    }
...
function CategoryView(builder: ComponentV2Builder, state: ISetupState): void {
    ...
    builder.roleSelect({ customId: `${SETUP_PREFIX}:categoryrole`, placeholder: "🛠️ | Eigene Rolle zuweisen …" });

    builder.buttons(
        { customId: `${SETUP_PREFIX}:editcategory`, label: "Text ändern", emoji: "✏️", tone: "primary" },
        { customId: `${SETUP_PREFIX}:allroles`, label: "Alle Rollen", emoji: "🛠️" },
...
function Panel(builder: ComponentV2Builder, state: ISetupState): void {
    ...
    builder.buttons(
        ...
        { customId: `${SETUP_PREFIX}:editimage`, label: "Bild & Farbe", emoji: "🎨", tone: "primary" },
...
export function RenderSetup(_client: BotClient, state: ISetupState): ISetupPanelView {
    ...
    else if (state.view === "roles") Roles(builder, state);
    else if (state.view === "categories") Categories(builder, state);
    else if (state.view === "category") CategoryView(builder, state);
